...
透過兩個新特性（trait），`Slice` 跟 `SliceMut`.

它還將 範圍`配對`模式的符號改為 `...`，以表示它們是包含的，而 `..` 在切片中是排除的。
...
在切片操作，特別是 `as_slice`，是相當常見的基本操作，用來處理 vector，也可以用來處理其他類型的容器。我們已經有了透過 `Index` 特性進行索引的符號，此 RFC 本質上是這特性的延續。
...
我們正在逐漸走向一個類似 Python 的世界，像是 `foo[n]` 會調用 `fail!`，當 `n` 超出範圍時，而相應的方法像是 `get` 則返回 `Option` 而不是失敗。透過為切片提供類似的符號，我們打開了在整個 vector-like APIs 中遵循相同協定的大門。
...
此設計是對 `Index` trait 設計的直接延續。我們引入了兩個新特性（trait），用於不可變與可變切片：
...
另一方面，這種方法對於切片是有問題的，因為一般來說，他會產生一個 unsized 類型（在 DST 中），當然，切片是為了給你一個指向切片大小的胖指針，我們不希望立即取消引用。但是，這樣的結果是，我們需要預先知道切片的可變性，因為它決定了表達式的類型。
...
一種可能性是重新引入某種自動切片的強制。我們曾經有一個從（）`Vec<T>` 到 `&[T]` 的強制轉換。由於我們不再強制擁有借來的值，我們現在可能想要一個強制 `&Vec<T>` 到 `&[T]` 的轉換：  
