...
高效能網絡服務經常使用非同步 IO，而不是阻塞 IO，因為在處理許多並行連接時更容易獲得最佳效能。 Rust 已經在網路服務領域得到了一些採用，我們希望透過使 Rust 中撰寫非同步網路服務更符合人因工程學，繼續支援這些使用者 - 並支援其他使用者採用。
...
point.
非同步函式的回傳型別是編譯器生成的唯一匿名型別，類似於閉包的型別。你可以把這種型別想像成一個枚舉，函式的每個「yield point」都有一個變體 —— 它的開頭、await 表達式和每個回傳。每個變體都儲存需要儲存的狀態，以便從該 yield point 恢復控制。
...
調用函式時，此匿名型別以其初始狀態返回，其中包含此函式的所有參數。
...
once will panic.
匿名返回型別實作 `Future`，`Item` 為它的回傳型別。輪詢它會推進函數的狀態，當它到達 `await` 時返回 `Pending` ，當它到達 `return` 時返回 `Ready` 。任何在它已經返回 `Ready` 一次後對其嘗試進行輪詢都會造成恐慌。
