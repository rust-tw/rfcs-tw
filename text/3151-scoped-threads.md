...
在標準函式庫中新增作用域執行緒，其允許借用父執行緒的變數來產生執行緒。
...
現在我們不必再手動加入執行緒，因為所有未加入的執行緒將在作用域結束時自動會合：
...
* 將作用域執行緒保留在外部 crates 之中。
  ...
  * 作用域執行緒通常是比 `thread::spawn()` 更好的預設值，因為它們確保生成的執行緒被連接並且不會意外 「洩漏」。這有時是單元測試中的一個問題，如果單元測試產生執行緒並忘記加入它們，「迷途」 的執行緒可能會積累。
